...
export function bindSponsorCreateButton() {
  ...
  if (!btnCreateSponsor) return;

  const canCreate = canManageEntity("sponsors", "create");

  btnCreateSponsor.hidden = !canCreate;
  btnCreateSponsor.disabled = !canCreate;

  btnCreateSponsor.onclick = (event) => {
    event.preventDefault();
    event.stopPropagation();

    if (!canManageEntity("sponsors", "create")) {
      showToast("No tenés permisos para crear sponsors");
      return;
    }
